...
def app():
    ...
    choice=st.selectbox('How do you want to enter data to summarise.',('Image','URL','Text'))
    if (choice=="Image"):
        uploaded_file = st.file_uploader("Choose an image...", type=["png",'jpg','jpeg'])
        if uploaded_file is not None:
            image = Image.open(uploaded_file)
            st.image(image, caption='Uploaded Image.', use_column_width=True)
            leng = st.selectbox('Type of Summarization:',('Large','Medium','Small'))
            if leng=='Large':
               _sum_len=70
            if leng=='Medium':
                _sum_len=50
            ...
            if button:
                ...
                text= summarize_para(text,_sum_len)
                st.write(text)
    if(choice=='URL'):
        ...
        button = st.button('Summarize')
        if button:
            text_url=summarize_url(URL,sum_len)
            st.write(text_url)

    if(choice=='Text'):
        text_= st.text_area("Enter the Text:",height=400)
        leng = st.selectbox('Type of Summarization:',('Large','Medium','Small'))
        if leng=='Large':
            sum_len_=70
        if leng=='Medium':
        ...
        button = st.button('Summarize')
        if button:
            text_para= summarize_para(text_,sum_len_)
            st.write(text_para)